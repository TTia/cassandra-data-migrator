...
/**
 * Factory for creating PostgresCopyJobSession instances.
 * Used when the target database type is PostgreSQL.
 */
public class PostgresCopyJobSessionFactory implements Serializable {
    ...
    /**
     * Gets a singleton instance of PostgresCopyJobSession.
     *
     * @param originSession the Cassandra origin session
     * @param propHelper    the property helper
     * @return the job session instance
     */
    public PostgresCopyJobSession getInstance(CqlSession originSession, PropertyHelper propHelper) {
        if (jobSession == null) {
